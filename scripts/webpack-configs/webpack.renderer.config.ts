...
function overrideJestConfigs(config: Config.InitialOptions): Config.InitialOptions {
    ...
    config.roots = ['<rootDir>/src/renderer', '<rootDir>/src/shared'];

    // 3. The option `collectCoverageFrom` is `['src/**/*.{js,jsx,ts,tsx}', '!src/**/*.d.ts']`, change it.

    config.collectCoverageFrom = ['src/renderer/**/*.{ts,tsx}', 'src/shared/**/*.{ts,tsx}', '!src/**/*.d.ts'];
    ...
    config.testMatch = [
        ...
        '<rootDir>/src/shared/**/__tests__/**/*.{ts,tsx}',
        '<rootDir>/src/shared/**/*.{spec,test}.{ts,tsx}',
    ];
