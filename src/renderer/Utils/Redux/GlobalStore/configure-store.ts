export interface ConfigureGlobalStoreOptions<TGlobalState> {
    initialGlobalState: TGlobalState;
}

/**
 * The global store, the extra part of Redux store, is used only for performance.
 *
 ...
 * global store to avoid the time cost.
 *
 ...
 */
export function configureGlobalStore<TGlobalState>(options: ConfigureGlobalStoreOptions<TGlobalState>) {
    const globalState: TGlobalState = options.initialGlobalState;
